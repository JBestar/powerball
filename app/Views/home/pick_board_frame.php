<?php
/**
 * mainFrame 전용 — 선배님 분석픽공유 게시판 (community 스킨 테이블)
 * 참고: https://powerballgame.co.kr/bbs/board.php?bo_table=pick
 */
$local = $local ?? rtrim(site_furl(''), '/');
$cssVer = $cssVer ?? '1';
$bo_table = $bo_table ?? 'pick';
$page = (int) ($page ?? 1);
...
$login_uid = $login_uid ?? '';
$is_pick_admin = $is_pick_admin ?? false;
$wr_id = (int) ($wr_id ?? 0);
$read_post = $read_post ?? null;
$pick_notice = $pick_notice ?? null;
$pick_newer_id = isset($pick_newer_id) ? ($pick_newer_id === null ? null : (int) $pick_newer_id) : null;
$pick_older_id = isset($pick_older_id) ? ($pick_older_id === null ? null : (int) $pick_older_id) : null;
...
<head>
	...
	<title><?= esc($site_title ?? '분석픽공유') ?></title>
...
<div id="bo_list" style="width:100%">
	<div class="categoryTit">
		<ul style="background-color:#F1F1F1; overflow:hidden;">
			...
			<li><a href="<?= esc($buildUrl(['page' => 1, 'sfl' => 'wr_subject', 'stx' => '', 'wr_id' => null])) ?>" class="on">분석픽공유</a></li>
	...
	<div id="bo_v_top">
		<ul class="bo_v_nb">
			<?php if ($pick_newer_id): ?>
			<li><a href="<?= esc($buildUrl(array_merge($listBaseQuery, ['wr_id' => $pick_newer_id]))) ?>" class="btn_b01">이전글</a></li>
			<?php endif; ?>
			<?php if ($pick_older_id): ?>
			<li><a href="<?= esc($buildUrl(array_merge($listBaseQuery, ['wr_id' => $pick_older_id]))) ?>" class="btn_b01">다음글</a></li>
			<?php endif; ?>
		</ul>
		<ul class="bo_v_com">
			<li><a href="<?= esc($buildUrl($listBaseQuery)) ?>" class="btn_b01">목록</a></li>
			<li><a href="#" class="btn_b02" onclick="alert('글쓰기는 추후 오픈 예정입니다.'); return false;">글쓰기</a></li>
		</ul>
	</div>
	<?php endif; ?>
	...
	<div class="tbl_head01 tbl_wrap">
		<table>
		<caption class="sound_only">분석픽공유 목록</caption>
		<colgroup>
			...
			<col style="width:60px">
			<col style="width:60px">
			<?php if ($is_pick_admin): ?><col style="width:88px"><?php endif; ?>
		</colgroup>
		<thead>
		<tr>
			<th scope="col">번호</th>
			...
			<?php if ($is_pick_admin): ?><th scope="col">관리</th><?php endif; ?>
		</tr>
		</thead>
		<tbody>
		<?php if ($pick_notice && $page === 1) :
		    $nid = (int) ($pick_notice->id ?? 0);
		    $ntitle = (string) ($pick_notice->title ?? '');
		    $ncc = (int) ($pick_notice->comment_count ?? 0);
		    $nUid = (string) ($pick_notice->mb_uid ?? '');
		    $nmb = trim((string) ($pick_notice->mb_nickname ?? ''));
		    if ($nmb === '') {
		        $nmb = $nUid !== '' ? $nUid : '운영자';
		    }
		    $nhit = (int) ($pick_notice->wr_hit ?? 0);
		    $ngood = (int) ($pick_notice->wr_good ?? 0);
		    $nTime = $pick_notice->created_at ?? '';
		...
		<tr class="bo_notice">
			...
			<?php if ($is_pick_admin): ?><td></td><?php endif; ?>
		</tr>
		<?php endif; ?>
		<?php
		...
		<tr class="<?= $rowRead ? 'bo_read' : '' ?>">
			...
			<?php if ($is_pick_admin): ?>
			<td class="td_mng" style="text-align:right; white-space:nowrap; font-size:10px;">—</td>
			<?php endif; ?>
		</tr>
		<?php endforeach; ?>
		<?php if ($total === 0 && count($rows) === 0 && !$pick_notice): ?>
		<tr><td colspan="<?= $is_pick_admin ? 7 : 6 ?>" style="text-align:center;padding:24px;color:#999;">등록된 글이 없습니다.</td></tr>
		<?php endif; ?>
		</tbody>
		</table>
	</div>
...
<fieldset id="bo_sch">
	...
	<form name="fsearch" method="get" action="<?= esc(site_furl('frame/communityBoard')) ?>">
		<input type="hidden" name="bo_table" value="pick">
