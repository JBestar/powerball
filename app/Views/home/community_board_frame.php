<?php
/**
 * mainFrame 전용 — 선배님 유머 게시판: 읽기(bo_v) + 목록(bo_list) 한 화면
 * 참고: https://powerballgame.co.kr/bbs/board.php?bo_table=humor&wr_id=1
 */
$local = $local ?? rtrim(site_furl(''), '/');
$cssVer = $cssVer ?? '1';
$bo_table = $bo_table ?? 'humor';
$page = (int) ($page ?? 1);
...
$latest_notice = $latest_notice ?? null;
$wr_id = (int) ($wr_id ?? 0);
$read_post = $read_post ?? null;
$humor_newer_id = isset($humor_newer_id) ? ($humor_newer_id === null ? null : (int) $humor_newer_id) : null;
$humor_older_id = isset($humor_older_id) ? ($humor_older_id === null ? null : (int) $humor_older_id) : null;
...
<head>
	...
	<title><?= esc($site_title ?? '유머게시판') ?></title>
	...
	<style>
		/* default.css 의 .sound_only(position:absolute;top:0;left:0 !important) 가 pg_current 숫자와 겹침 → 화면 밖으로 이동 */
		#bo_list .sound_only,
		...
		nav.community-pg a.pg_page {
			vertical-align: middle !important;
			line-height: 22px !important;
		}
		/* bbs.css fieldset { font-size:0 } 보완 */
	...
	<?php endif; ?>
	<script>
	/** mainFrame 높이: scrollHeight 기준(본문 펼침·목록+페이징 전체 포함). viewimageresize는 load 이후 레이아웃 변경 → 지연 재측정 */
...
<div id="bo_list" style="width:100%">
	<div class="categoryTit">
		<ul style="background-color:#F1F1F1; overflow:hidden;">
			<li><a href="<?= esc($buildUrl(['page' => 1, 'sfl' => 'wr_subject', 'stx' => '', 'wr_id' => null])) ?>" class="on">유머</a></li>
		...
		<section id="bo_v_atc">
			...
			<div id="bo_v_act">
				<span class="bo_v_act_gng">
					<a href="#" id="good_button" onclick="return false;"><strong>0</strong><span>추천</span></a>
	...
	<div id="bo_v_top">
		<ul class="bo_v_nb">
			<?php if ($humor_newer_id): ?>
			<li><a href="<?= esc($buildUrl(array_merge($listBaseQuery, ['wr_id' => $humor_newer_id]))) ?>" class="btn_b01">이전글</a></li>
			<?php endif; ?>
			<?php if ($humor_older_id): ?>
			<li><a href="<?= esc($buildUrl(array_merge($listBaseQuery, ['wr_id' => $humor_older_id]))) ?>" class="btn_b01">다음글</a></li>
			<?php endif; ?>
		</ul>
		<ul class="bo_v_com">
			<li><a href="<?= esc($buildUrl($listBaseQuery)) ?>" class="btn_b01">목록</a></li>
			<li>
				<a href="#" class="btn_b02" onclick="<?php if ($isLogin): ?>window.open('<?= esc(site_furl('/?view=humorRegister')) ?>','humorRegister','width=600,height=650');<?php else: ?>alert('로그인 후 이용가능합니다.');<?php endif ?> return false;">글쓰기</a>
			</li>
			<?php if ($is_humor_admin && $rpId > 0): ?>
			<li><a href="#" class="btn_b01" onclick="window.open('<?= esc(site_furl('/?view=humorEdit&id=' . $rpId)) ?>','humorEdit','width=600,height=650'); return false;">수정</a></li>
			<li><a href="<?= esc(site_furl('/?view=humorDelete&id=' . $rpId)) ?>" class="btn_b01" onclick="return confirm('정말 삭제하시겠습니까?');">삭제</a></li>
			<?php endif; ?>
		</ul>
	</div>
	<?php endif; ?>
	...
	<div class="tbl_head01 tbl_wrap">
		<table>
		<caption class="sound_only">유머게시판 목록</caption>
		<colgroup>
			...
			<col style="width:60px">
			<col style="width:60px">
			<?php if ($is_humor_admin): ?><col style="width:88px"><?php endif; ?>
		</colgroup>
		<thead>
		<tr>
			<th scope="col">번호</th>
			...
			<?php if ($is_humor_admin): ?><th scope="col">관리</th><?php endif; ?>
		</tr>
		</thead>
		<tbody>
		<?php if ($latest_notice) :
		    $nFid = (int) ($latest_notice->notice_fid ?? 0);
		    $nTitle = (string) ($latest_notice->notice_title ?? '');
		    $nNick = (string) ($latest_notice->mb_nickname ?? '');
		    if ($nNick === '') {
		        $nNick = (string) ($latest_notice->notice_mb_uid ?? '운영자');
		    }
		    $nGrade = (int) ($latest_notice->mb_grade ?? 30);
		    if ($nGrade < 0) {
		        $nGrade = 0;
		    }
		    if ($nGrade > 20) {
		    ...
		    $classGif = site_furl('images/class/M' . $nGrade . '.gif');
		    $nTime = $latest_notice->notice_time_create ?? '';
		    $nDateShow = $nTime ? date('m-d', strtotime((string) $nTime)) : '—';
		    $nHit = number_format((int) ($latest_notice->notice_hit ?? 0));
		    ?>
		<tr class="bo_notice">
			<td class="td_num"><img src="<?= esc($icoNotice) ?>" style="vertical-align:top;" alt="공지"></td>
			<td class="td_subject">
				<a href="<?= esc(site_furl('frame/customerCenter?id=' . $nFid)) ?>">
					<?= esc($nTitle !== '' ? $nTitle : '공지') ?>
				</a>
			</td>
			<td class="td_name sv_use"><img src="<?= esc($classGif) ?>" alt="" onerror="this.style.display='none'"> <span class="sv_member"><?= esc($nNick) ?></span></td>
			<td class="td_date"><?= esc($nDateShow) ?></td>
			<td class="td_num"><?= esc($nHit) ?></td>
			<td class="td_num">—</td>
			<?php if ($is_humor_admin): ?><td></td><?php endif; ?>
		</tr>
		<?php endif; ?>
		<?php
		...
		<tr class="<?= $rowRead ? 'bo_read' : '' ?>">
			...
			<td class="td_mng" style="text-align:right; white-space:nowrap; font-size:10px;">
				<a href="#" onclick="window.open('<?= esc(site_furl('/?view=humorEdit&id=' . $hid)) ?>','humorEdit','width=600,height=650'); return false;" style="color:#0e609c;">수정</a>
				<a href="<?= esc(site_furl('/?view=humorDelete&id=' . $hid)) ?>" onclick="return confirm('정말 삭제하시겠습니까?');" style="color:#c11a20; margin-left:4px;">삭제</a>
			</td>
			<?php endif; ?>
		</tr>
		<?php endforeach; ?>
		<?php if ($total === 0 && count($rows) === 0): ?>
		<tr><td colspan="<?= $is_humor_admin ? 7 : 6 ?>" style="text-align:center;padding:24px;color:#999;">등록된 글이 없습니다.</td></tr>
		<?php endif; ?>
		</tbody>
		</table>
	</div>
